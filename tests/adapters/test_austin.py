...
def test_fetch_passes_date_window_to_where_clause():
    ...
    responses.add(
        responses.GET, BASE,
        json=[{"meeting_date": "2026-05-27T00:00:00.000"}], status=200,
    )
...
def test_fetch_returns_empty_list_when_window_is_quiet_but_dataset_is_fresh():
    """A week with no votes is normal and must not raise, as long as the
    dataset has been updated recently relative to the window."""
    responses.add(responses.GET, BASE, json=[], status=200)
    responses.add(
        responses.GET, BASE,
        json=[{"meeting_date": "2026-06-10T00:00:00.000"}], status=200,
    )
...
def test_fetch_raises_when_dataset_has_gone_stale():
    """This is the bug found in production: 3c89-i35a stopped receiving new
    rows after 2026-05-28, and every windowed query since has returned []
    while the digest reported "Austin Council: ok". A stale dataset must
    now surface as a loud failure instead of a silent empty list."""
    responses.add(responses.GET, BASE, json=[], status=200)
    responses.add(
        responses.GET, BASE,
        json=[{"meeting_date": "2026-05-28T00:00:00.000"}], status=200,
    )
